...
void llcm_concurrent_queue_init(struct llcm_concurrent_queue *,
                                size_t capacity);
void llcm_concurrent_queue_init_with_custom_allocate(
    struct llcm_concurrent_queue *, size_t capacity, struct llcm_allocator);
...
bool llcm_concurrent_queue_try_reserve_size_before_push(
    struct llcm_concurrent_queue *, size_t num_new_entries);
// can be called after an entry is popped and will not be pushed again
void llcm_concurrent_queue_unreserve_size_after_pop(
    struct llcm_concurrent_queue *, size_t num_old_entries);
...
static_assert(sizeof(struct llcm_concurrent_queue_entry) ==
                  LLCM_CONCURRENT_QUEUE_CACHE_LINE_SIZE,
              "");

size_t
llcm_concurrent_queue_get_capacity(struct llcm_concurrent_queue const *queue) {
    return queue->mask + 1;
}

void llcm_concurrent_queue_init(struct llcm_concurrent_queue *queue,
                                size_t capacity) {
    llcm_concurrent_queue_init_with_custom_allocate(
        queue, capacity, llcm_allocator_create_default());
}

void llcm_concurrent_queue_init_with_custom_allocate(
    struct llcm_concurrent_queue *queue, size_t capacity,
...
bool llcm_concurrent_queue_try_reserve_size_before_push(
    struct llcm_concurrent_queue *queue, size_t num_new_entries) {
    uint64_t const reserved_push_size = __atomic_fetch_add(
        &queue->reserved_push_size, num_new_entries, __ATOMIC_SEQ_CST);
    if (reserved_push_size > queue->mask) {
        __atomic_fetch_sub(&queue->reserved_push_size, num_new_entries,
                           __ATOMIC_SEQ_CST);
        return false;
    }
    return true;
...
void llcm_concurrent_queue_unreserve_size_after_pop(
    struct llcm_concurrent_queue *queue, size_t num_old_entries) {
    __atomic_fetch_sub(&queue->reserved_push_size, num_old_entries,
                       __ATOMIC_SEQ_CST);
}

void llcm_concurrent_queue_push(struct llcm_concurrent_queue *queue,
                                void *value) {
...
void *llcm_concurrent_queue_try_pop(struct llcm_concurrent_queue *queue) {
    ...
    while (NULL == read_value) {
        read_value =
            __atomic_exchange_n(&read_entry->entry, NULL, __ATOMIC_SEQ_CST);
    }
    return read_value;
}