...
void *thread_exec(void *arg0) {
    ...
    ConcurrentQueue<uint64_t> *queue = thread_args->queue;
...
struct test_result multithreaded_test(struct test_config config) {
    ...
    ConcurrentQueue<uint64_t> queue(config.num_elements);
    queue.TryReserveSizeBeforePush(config.num_elements);
    ...
    queue.UnreserveSizeAfterPop(config.num_elements);
