...
inline void set_affinity_(int cpu) {
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(cpu, &cpuset);
...
inline void set_realtime_priority_() {
...
inline void thread_perf_mode_main_thread_init() { set_affinity_(1); }

inline void thread_perf_mode_init(int cpu) {
...
inline uint64_t diff_timespec(const struct timespec *time1, const struct timespec *time0) {
