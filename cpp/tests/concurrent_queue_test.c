...
void basic_queue_test() {
    ...
    ConcurrentQueue<void *> queue(capacity);
    ...
    for (uint64_t i = 1; i <= capacity * 4; i++) {
        ...
        queue.Push((void *) i);
        void *pop_result = queue.TryPop().value();
...
struct thread_args {
    ConcurrentQueue<void *> *queue_ = nullptr;
...
void *thread_exec(void *arg0) {
    ...
    for (uint64_t sequence_number = 1; sequence_number < MULTITHREADED_TEST_MAX_SEQUENCE;
         sequence_number++) {
        args->queue_->Push((void *) sequence_number);
        std::optional<void *> pop_result = std::nullopt;
        while (std::nullopt == pop_result) {
            pop_result = args->queue_->TryPop();
        }
        args->sequence_tracker_->MarkObserved((uint64_t) pop_result.value());
    }
    return NULL;
}

void multithreaded_test() {
    ConcurrentQueue<void *> queue(MULTITHREADED_TEST_NUM_THREADS);
